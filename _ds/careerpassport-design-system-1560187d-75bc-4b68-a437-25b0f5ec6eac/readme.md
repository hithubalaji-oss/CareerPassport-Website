...
is **Navy & Gold Foil**: `--emerald-deep` #10203f (navy ink) · `--emerald-mid` #1b3564 (cover navy) ·
`--emerald` #d8b877 (gold) · `--mint` #e9d6aa · `--emerald-tint` #fdfaf1 (paper). `--emerald` is the action colour — primary buttons fill
with it and set their label in `--emerald-deep` — and also the focus ring, strong border and
`--success` alias. Links run on their own stamp-blue tier: `--link` #7ba4ff, `--link-deep` #b9cdff
on press, `--link-soft` #0f1a33 as the wash behind verified chips. Semantics are lifted for dark
surfaces: error #f2544a on a #2b1614 wash, warning #e8a93a on #2a2113.
...
**Hover and press.** Gold fills brighten to #e6c795; panels lift one step to #1c211f;
nav links gain that same lift behind a fully-rounded hit area; links move to `--link-deep` (#b9cdff,
